                        ...
                        <tbody>
                            @if(old('details'))
                                ...
                                <tr id="tr_{{$key}}" class="tr_pro">
                                    <td>
                                        <input type="text" name="details[{{$key}}][product]"  placeholder="Search product by name ..." class="form-control select-md"  id="product_name{{$key}}" onkeyup="getProductByNameWithStock(this.value, {{$key}});" value="{{ old('details.'.$key.'.product') }}" > 
                            ...
                            @else
                                ...
                                <tr id="tr_{{$i}}" class="tr_pro">
                                    <td>
                                        <input  type="text" name="details[{{$i}}][product]" placeholder="Search product by name ..." class="form-control select-md"  id="product_name{{$i}}" value="" onkeyup="getProductByNameWithStock(this.value, {{$i}});" autocomplete="off"> 
...
<script>
    ...
    // }
    function getProductByNameWithStock(name, count) {  
        if(name.length > 0) {
            $.ajax({
                url: "{{ route('admin.product.searchByNameForStock') }}",
                method: 'post',
                data: {
                    '_token': '{{ csrf_token() }}',
                    term: name,
                ...
                success: function(result) {
                    ...
                    if (result.length > 0) {
                        ...
                        $.each(result, (key, value) => {
                            // Add pcs and ctn values, make sure they exist in the result object
                            content += `<a class="dropdown-item" href="javascript: void(0)" onclick="fetchProductPcsPerCtn('${count}',${value.id},${value.pcs})">
                                            ${value.name} 
                                            <span class="product-info">
                                                pcs: ${value.pcs} 
                                             <span class="product-info">
                                                pcs: ${value.pcs} 
                                                <span class="text-${value.ctn > 0 ? 'success' : 'danger'}">ctn: ${value.ctn}</span>
                                            </span>
                                            </span>
                                        </a>`;
                        });
    ...
    //             if(result.warning){
    //                 $("#propcs" + count).val(result.total_pcs);
    //             }else{
    ...
    //     });                
    // }
    function fetchProductPcsPerCtn(count,id, pcs) {
        $('.product-dropdown').hide()
        var store_id = $('#store_id').val();
        $.ajax({
            url: "{{ route('admin.product.viewDetailPcsPerCtn') }}",
            method: 'post',
            dataType: 'json',
            data: {
                '_token': '{{ csrf_token() }}',
